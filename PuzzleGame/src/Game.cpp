...
//	}
	...
	//	Shader* characterShader = new Shader("Shaders/normalVShader.glsl", "Shaders/normalFShader.glsl");
	//	m_ShaderList.push_back(characterShader);
	...
	//	toon = new Character(new AnimationModel("ObjFiles/Slime.dae", "ObjFiles/Slime-taunt.dae", "Textures/AlbedoSlimer.png", "Textures/NormalSlimer.png", m_ShaderList[2]));
	//	toon->SetTransform(glm::vec3(1.1f, -1.f, 0.0f), glm::vec4(0.f, 1.f, 0.f, 80.f), glm::vec3(0.01f));
	...
	//	model = toon->GetTransform();
	//	mvp = proj * view * model;
	//	m_ShaderList[2]->SetMat4f(model, "M");
	//	m_ShaderList[2]->SetMat4f(glm::transpose(glm::inverse(model)), "NormalMatrix");
	//	m_ShaderList[2]->SetMat4f(mvp, "MVP");
	//	toon->Render(pass);
	//}
	...
	//	m_Light.SendToShader(*m_ShaderList[2]);

	//	shader->Set1i(m_PointLights.size(), "pointLightCount");
	//	m_ShaderList[2]->Set1i(m_PointLights.size(), "pointLightCount");
	//	for (size_t i = 0; i < m_PointLights.size(); ++i) {
	//		m_PointLights[i].SendToShader(*shader, i);
	//		m_PointLights[i].SendToShader(*m_ShaderList[2], i);
	...
	//		m_ShaderList[2]->SetVec3f(lightPosEyeSpace, ("LightPositionsEyeSpace[" + std::to_string(i) + "]").c_str());
	//	}

	//	shader->Set1i(m_SpotLights.size(), "spotLightCount");
	//	m_ShaderList[2]->Set1i(m_SpotLights.size(), "spotLightCount");
	//	for (size_t i = 0; i < m_SpotLights.size(); ++i) {
	//		m_SpotLights[i].SendToShader(*shader, i);
	//		m_SpotLights[i].SendToShader(*m_ShaderList[2], i);
	//	}

	//	shader->SetMat4f(m_MainCamera.CalculateViewMatrix(), "V");
	//	m_ShaderList[2]->SetMat4f(m_MainCamera.CalculateViewMatrix(), "V");
	//	shader->SetVec3f(m_MainCamera.GetPosition(), "cameraPos");
	//	m_ShaderList[2]->SetVec3f(m_MainCamera.GetPosition(), "cameraPos");
	//	shader->SetMat4f(m_Light.CalculateLightTransform(), "directionalLightTransform");
	//	m_ShaderList[2]->SetMat4f(m_Light.CalculateLightTransform(), "directionalLightTransform");

	//	m_Light.GetShadowMap()->Read(GL_TEXTURE0 + 2);
	//	shader->Set1i(2, "directionalShadowMap");
	//	m_ShaderList[2]->Set1i(2, "directionalShadowMap");
