...
vec3 CalcRimLight(vec3 color, vec3 normal, int intensity)
{
	vec3 toEye = normalize(cameraPos - fs_in.v_Position);
	float rim = 1 - dot(normal, toEye);
	rim = clamp(rim - 0.7, 0.0, 1.0);
	return (rim * color * intensity);
}
...
vec4 CalcLight(Light light, vec3 direction, vec3 normal, float shadowFactor)
{
	...
	if (diffuseFactor > 0.0)
	{
		...
		if (IsSelected) {
			vec3 selectionLighting = CalcRimLight(vec3(1.0, 1.0, 0.0), normal, 50);
			diffuseColor += vec4(selectionLighting, 0.8);
		}
		...
		if (dot(toEyePos, normalize(normal)) <  mix(0.2, 0.3, dot(normalize(direction), normalize(normal))) && !IsSelected) {
			return vec4(0.0, 0.0, 0.0, 1.0);
		} 
...
// Air to glass ratio for refraction (Eta)
//const float Eta = 1.0 / 1.51714;
...
void main()
{
	...
	// --- Invisibility Stuff ---
	//vec3 incident = normalize(vec3(fs_in.v_Position-cameraPos));
	//vec3 v_refraction = refract(incident, Normal, Eta);
	//vec3 v_reflection = reflect(incident, Normal);
	//vec4 refractionColor = vec4(0.1, 0.1, 0.1, 0.1) * vec4(normalize(v_refraction), 0.1);
	//vec4 reflectionColor = vec4(0.1, 0.1, 0.8, 0.1) * vec4(normalize(v_reflection), 0.1);
	//out_color = mix(vec4(refractionColor.xyz, 0.1), vec4(reflectionColor.xyz, 0.1), dot(incident, Normal));// + vec4(selectionLighting, 0.1); 
	//mix(vec4(0.1, 0.1, 0.1, 0.1), vec4(selectionLighting, 0.5), v_fresnel);
	// --- End Invisibility Stuff ---

	out_color = frag_color * light_color;
	if (IsSelected) {
		vec3 selectionLighting = CalcRimLight(vec3(1.0, 1.0, 1.0), Normal, 50);
		out_color += vec4(selectionLighting, 1.0);
	}	
}